...
namespace ge {
	...
	public:
		...
		struct material_s {
			uint32_t index_albedo = 0; 
			uint32_t index_normal = 0; 
			uint32_t index_metallic = 0; 
			uint32_t index_roughness = 0;
			uint32_t index_emissive = 0; 
			uint32_t index_occlusion = 0; 
			uint32_t index_debug = 0;
			uint32_t light_equation = 0;
		};
